const LANGUAGES = [
  { code:'fil', label:'Filipino', native:'Wikang pambansa' },
  { code:'en',  label:'English', native:'Default for medical terms' },
  { code:'ceb', label:'Cebuano', native:'Bisaya · Sugbuanon' },
  { code:'ilo', label:'Ilocano', native:'Iloko' },
  { code:'hil', label:'Hiligaynon', native:'Ilonggo' },
  { code:'war', label:'Waray', native:'Waray-Waray' },
  { code:'bcl', label:'Bikol', native:'Bikolano' },
  { code:'pam', label:'Kapampangan', native:'Pampango' },
  { code:'pag', label:'Pangasinan', native:'Pangasinense' },
  { code:'mdh', label:'Maguindanaon', native:'Mindanao' },
  { code:'mrw', label:'Maranao', native:'Lanao' },
  { code:'tsg', label:'Tausug', native:'Sulu' },
];
let currentReportLang = 'fil';

function renderLanguageList(){
  const wrap = document.getElementById('lang-list');
  wrap.innerHTML = LANGUAGES.map(l => `
    <button class="setting-row lang-opt" style="width:100%; background:none; border:none; text-align:left; cursor:pointer;" onclick="selectReportLanguage('${l.code}')">
      <div class="lbl" style="display:flex; flex-direction:column; align-items:flex-start; gap:2px;">
        <span style="font-weight:600;">${l.label}</span>
        <span style="font-size:10.5px; font-weight:400; color:var(--ink-500);">${l.native}</span>
      </div>
      <div class="val">${l.code === currentReportLang ? checkIcon() : ''}</div>
    </button>
  `).join('');
}

function checkIcon(){
  return '<svg viewBox="0 0 24 24" fill="none" width="18" height="18"><circle cx="12" cy="12" r="10" fill="var(--ember-500)"/><path d="M7.5 12.5l3 3 6-6.5" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
}

function selectReportLanguage(code){
  currentReportLang = code;
  const lang = LANGUAGES.find(l => l.code === code);
  document.getElementById('report-lang-val').textContent = lang.label;
  renderLanguageList();
  setTimeout(()=>{ go('profile'); }, 260);
}
...
const TESTS = {
  hgb:{
   ...
   technical:{en:"Hemoglobin (Hgb) measures the oxygen-carrying protein in red blood cells. 14.5 g/dL falls within the standard reference interval of 12.0–16.0 g/dL for adults, indicating adequate oxygen-carrying capacity.",
    ...
    tips:{
      en:["Iron-rich foods like leafy greens, beans, and lean meat help maintain healthy hemoglobin.","Pairing iron-rich meals with vitamin C (citrus, tomatoes) helps your body absorb iron better.","No follow-up needed right now — this result is reassuring."],
      fil:['Ang mga pagkaing mayaman sa iron gaya ng gulay na madahon, beans, at karneng walang taba ay tumutulong panatilihing malusog ang hemoglobin.','Ang pagsasama ng iron-rich na pagkain sa vitamin C (citrus, kamatis) ay nagpapadali sa pagsipsip ng iron.','Walang kailangang gawin sa ngayon — magandang resulta ito.']
    }
  },
  wbc:{
    ...
    tips:{
      en:["Rest and stay hydrated — your body may be using extra energy fighting something off.","Watch for fever, unusual fatigue, or new symptoms and mention them at your next visit.","A repeat test in a few weeks can help confirm whether this was temporary."],
      fil:['Magpahinga at uminom ng sapat na tubig — posibleng gumagamit ang katawan mo ng dagdag na enerhiya para labanan ang isang bagay.','Bantayan ang lagnat, hindi pangkaraniwang pagkapagod, o bagong sintomas at ibahagi ito sa susunod na pagpapatingin.','Ang muling pagsusuri sa loob ng ilang linggo ay makakatulong kumpirmahin kung pansamantala lang ito.']
    }
  },
  glu:{
    ...
    tips:{
      en:["Short walks after meals can help your body manage blood sugar more easily.","Consider cutting back on sugary drinks and refined carbs until you talk with your doctor.","An HbA1c test shows your average blood sugar over months — ask your doctor if it's right for you."],
      fil:['Ang maikling paglalakad pagkatapos kumain ay makakatulong sa katawan na mas madaling pamahalaan ang asukal sa dugo.','Bawasan ang matamis na inumin at refined carbs hanggang makausap mo ang iyong doktor.','Ang HbA1c test ay nagpapakita ng average blood sugar mo sa nakalipas na mga buwan — itanong sa doktor kung angkop ito para sa iyo.']
    }
  },
  crea:{
    ...
    tips:{
      en:["Staying well-hydrated throughout the day supports healthy kidney function.","Routine follow-up tests help track your kidney health over time.","No action needed right now — this result is reassuring."],
      fil:['Ang sapat na pag-inom ng tubig sa araw-araw ay sumusuporta sa malusog na paggana ng bato.','Ang regular na follow-up test ay tumutulong bantayan ang kalusugan ng bato sa paglipas ng panahon.','Walang kailangang gawin sa ngayon — magandang resulta ito.']
    }
  },
  plt:{
    ...
    tips:{
      en:["Avoid activities with a high risk of falls or injury until you've discussed this with your doctor.","Watch for unusual bruising or bleeding and mention it at your next visit.","A repeat count can confirm whether this was just a one-time dip."],
      fil:['Iwasan ang mga aktibidad na may mataas na peligro ng pagkahulog o pinsala hangga\'t hindi mo pa napag-uusapan ito sa doktor.','Bantayan ang hindi pangkaraniwang pasa o pagdurugo at ibahagi ito sa susunod na pagpapatingin.','Ang muling pagsusuri ay makakatulong kumpirmahin kung pansamantala lang ito.']
    }
  }
};
let currentTest = 'wbc';
...
function renderTest(){
  ...
  const checkSvg = '<svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="var(--sand-100)"/><path d="M7.5 12.5l3 3 6-6.5" stroke="var(--green)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  const tipsEl = document.getElementById('d-tips');
  tipsEl.innerHTML = t.tips[currentLang].map(tip => `<li>${checkSvg}<span>${tip}</span></li>`).join('');
}
...
// build waveform bars once, then run initial render on load
